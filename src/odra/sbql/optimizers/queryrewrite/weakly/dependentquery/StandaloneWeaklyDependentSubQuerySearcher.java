...
import odra.sbql.optimizers.queryrewrite.weakly.dependentquery.LSCMethod.LSCDependencyChecker;
import odra.system.config.ConfigDebug;

public class StandaloneWeaklyDependentSubQuerySearcher extends TraversingASTAdapter{
	...
	public HashMap<String, Vector<Expression>> hmEnums;
	/**
	 * @param context - non-algebraic operator - the context for dependency 
	 * checking process
	 */
	public StandaloneWeaklyDependentSubQuerySearcher(NonAlgebraicExpression context) {
...
*/
	private boolean canLSCMethodBeApplied(Expression expr, OptimizationKind reqKind) throws SBQLException{
		...
		WeaklyDependencyChecker dependencyChecker = new WeaklyDependencyChecker(context);
		expr.accept(dependencyChecker, null);
		if(dependencyChecker.isWeaklyDependent()){
			this.hmEnums = dependencyChecker.getHmEnums();
			return true;
