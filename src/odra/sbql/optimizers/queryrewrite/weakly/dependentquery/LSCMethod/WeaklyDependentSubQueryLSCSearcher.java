...
public class WeaklyDependentSubQueryLSCSearcher extends TraversingASTAdapter {
	...
	public Object visitIfThenExpression(IfThenExpression expr,
		...
		if(this.canLSCMethodBeApplied(expr, (OptimizationKind)attr)){
			return expr;
		}
	...
	public Object visitIfThenElseExpression(IfThenElseExpression expr,
		...
		if(this.canLSCMethodBeApplied(expr, (OptimizationKind)attr)){
			...
			if(this.canLSCMethodBeApplied(expr.getConditionExpression(), (OptimizationKind)attr))
	 ...
	 * @param reqKind
	 */
	private boolean canLSCMethodBeApplied(Expression expr, OptimizationKind reqKind) throws SBQLException{
		
			LSCDependencyChecker dependencyChecker = new LSCDependencyChecker(context,globalModuleName);
			expr.accept(dependencyChecker, null);
			if(dependencyChecker.isLSCWeaklyDependent && !dependencyChecker.getDependentParts().isEmpty()){
				this.dependentParts = dependencyChecker.getDependentParts();
				return true;
